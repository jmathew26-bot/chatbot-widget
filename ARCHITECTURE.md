...
- [x] **Phase 6** -- `SchedulingAgent` + `GoogleCalendarProvider` implemented (scopes: `calendar.events` + `calendar.freebusy`, deliberately narrower than the blanket `calendar` scope); collision detection works today against the app's own Meeting table even without Google Calendar credentials. `network gmail-test`/`calendar-test` authenticate and verify access without sending/creating anything; `network gmail-test-send`/`calendar-test-create` are explicit, confirmation-gated commands for verifying a real send/create round-trips a message/thread/event ID correctly (the calendar one offers to delete its test event immediately after).
